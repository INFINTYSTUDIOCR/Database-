...
app.post('/alice', requireProductAuth, async (req, res) => {
  try {
    const { student, history, message, mode, secret, nexora, sessionType } = req.body || {};
    ...
    if (mode === 'start_session' || mode === 'return_session') {
      ...
      const companion = sessionType === 'companion';
      const greetInstruction = companion
        ? (returning
          ? `Welcome back ${display} briefly (max 2 sentences). Ask ONE open question like "What would you like to talk about today?" or "What's on your mind?" — let THEM choose the topic. Do NOT assign exercises.`
          : `First companion session: greet warmly using ONLY the name "${display}". Ask what they want to talk about today — e.g. "What are we going to talk about today?" Let them lead. No lesson plan, no connector drill.`)
        : (returning
      ...
      const openExtra = brainScopeExtra(student, req, `${mode}:${companion ? 'companion' : 'practice'}:${student?.level || 'Functional'}:${returning ? 'return' : 'new'}:${ALICE_BRAIN_VER}`);
      ...
      const resp = await claudeCall({
        ...
        messages: [{ role: 'user', content: `You are Alice (your name is ALICE, not Alaiz, not Alicia — always ALICE). You are a warm and encouraging English tutor using the Nexus Method. ${companion ? 'COMPANION MODE: conversational partner — student picks the topic.' : ''} ${greetInstruction} You are a tutor only — never roleplay as a customer, interviewer, or Nexora simulator.\n\nStudent level: ${student?.level||'Functional'}. Their exercises:\n${tb||'(none yet)'}${profileNote}${variation}\n\nEnd with: ALICE: [one motivating tip in Spanish]` }]
      });
    ...
    if (mode === 'evaluate') {
      ...
      const statsNote = companion
        ? `Companion session: ${metrics.turns} student turns, ${metrics.wordCount} words. Focus on fluency, confidence, and natural conversation — not connector drills. Computed score: ${overall_score}/100.`
        : `Session stats: ${metrics.turns} student turns, ${metrics.wordCount} words, connectors used: ${connectors_used.join(', ') || 'none'}. Computed score: ${overall_score}/100 — your feedback must match this performance level.`;

      const resp = await claudeCall({
        model: 'claude-haiku-4-5-20251001', max_tokens: 400,
        system: companion
          ? 'You are Alice in companion mode. Respond ONLY with valid JSON. Warm, conversational feedback — no overall_score field.'
          : 'You are Alice, a warm English tutor. Respond ONLY with valid JSON. No markdown. No overall_score field — score is already computed from the transcript.',
        messages: [{ role: 'user', content: companion
          ? `Evaluate this English companion conversation for ${student?.name || 'the student'} (level: ${student?.level || 'Functional'}).\n\n${statsNote}\n\nSession:\n${hist}\n\nReturn ONLY this JSON (no overall_score):\n{"best_moment":"One specific warm thing they did well in conversation","main_improvement":"One gentle tip for their chosen topic","alice_message":"2-3 warm encouraging sentences. End with: ALICE: [one motivating sentence in Spanish]"}`
          : `Evaluate this English practice session for ${student?.name || 'the student'} (level: ${student?.level || 'Functional'}).\n\n${statsNote}\n\nSession:\n${hist}\n\nReturn ONLY this JSON (no overall_score):\n{"best_moment":"One specific warm thing they did well","main_improvement":"One concrete tip tied to what they actually said","alice_message":"2-3 warm encouraging sentences. End with: ALICE: [one motivating sentence in Spanish]"}` }]
      });
...
const ALICE_COMPANION_RULES = `COMPANION MODE — conversational practice partner (not a drill sergeant):
- The student chose this session to talk freely. Follow THEIR topic with genuine curiosity and warmth.
- Open with interest in what they want to discuss. Do NOT assign exercises, scenarios, or connector drills unless they ask.
- Coach gently only when natural — if they make a mistake, model the correct phrase in your reply without lecturing.
- Do NOT force Idea + Linker + Idea every turn. Mention connectors only when it fits the conversation.
- Keep 2-4 natural sentences + ONE follow-up question about their topic.
- Wait for them — your question should invite them to speak, not test them.`;
...
app.post('/alice/stream', requireProductAuth, async (req, res) => {
  try {
    const { student, history, message, scenario, secret, sessionType } = req.body || {};
    if (req.auth.role === 'student' && !assertStudentScope(req, student?.id)) {
      return res.status(403).json({ error: 'Student scope mismatch' });
    }
    sanitizeStudentAiProfile(student);
