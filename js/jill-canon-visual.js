...
(function (global) {
  ...
  function loadConfig() {
    ...
    _load = fetch('config/jill-canon-visual.json?v=20260707g')
  ...
  function frameStyle() {
    ...
    if (img) {
      return 'background-color:' + bg + ';background-image:url(' + assetUrl(img) + '?v=20260707g);background-size:cover;background-position:center;';
    }
  ...
  function render(columnId, fallbackRef) {
    ...
    if (!clip) return '';
    var media = clip.gif ? assetUrl(clip.gif) + '?v=20260707g' : assetUrl(clip.svg) + '?v=20260707g';
    var alt = clip.title || (fallbackRef && fallbackRef.title) || 'Canon Jill';
    var tag = clip.gif ? 'img' : 'img';
    return '<div class="jill-canon-frame" style="margin-top:4px;border-radius:12px;overflow:hidden;border:1px solid rgba(61,220,151,0.35);' + frameStyle() + '">'
      + '<' + tag + ' src="' + media + '" alt="' + esc(alt) + '" style="display:block;width:100%;max-width:320px;height:auto;margin:0 auto;" loading="eager" decoding="async">'
      + '</div>';
  }
