...
const DJ_DIRECTIVE = {
  down: 'RITMO ? (baj· el tempo): el alumno viene fallando. UNA sola micro-correcciÛn + 1 frase modelo corta. PROHIBIDO re-ense?ar el guion completo, abrir tablero o inventar lecciÛn nueva.',
  hold: 'RITMO = (mantenÈ): una correcciÛn puntual + una pr·ctica del mismo tipo. Ritmo normal. Sin relleno.',
  up: 'RITMO ? (subÌ el tempo): viene limpio. Felicit· corto, subÌ la dificultad: UNA estructura nueva o una idea m·s larga, ritmo ·gil.'
};
...
function formatCoachNote(analysis, dj) {
  const lines = [
    ...
    '- Si hay un fallo, d· UNA micro-correcciÛn hablada y natural ("you went ? casi; decÌ I go conmigo"), model· la forma correcta en voz alta y seguÌ.',
    '- M·ximo 1 correcciÛn por turno. NO corrijas acento, tildes ni ortografÌa. CorrecciÛn con afecto firme, no chatbot motivacional.',
    '- PROHIBIDO llenar la pantalla con lecciones, paradigmas o tableros que el estudiante no pidiÛ.'
  ];
  if (analysis?.findings?.length) {
    lines.push('- PISTAS (posibles fallos detectados por el oÌdo): '
