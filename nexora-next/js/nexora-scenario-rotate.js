...
var NexoraScenarioRotate = (function () {
  ...
  function studentKey() {
    try {
      return localStorage.getItem('nexora_next_student_id') || 'anon';
    } catch (e) {
      return 'anon';
    }
  }
  ...
  function storageKey(nxConfig) {
    ...
    return 'nexora_next_rotate_v5_' + studentKey() + '_' + pk;
  }
  ...
  function loadSessionRecent(poolKey) {
    try {
      var raw = localStorage.getItem('nexora_next_recent_' + poolKey);
      if (!raw) return [];
  ...
  function saveSessionRecent(poolKey, id) {
    try {
      ...
      recent.unshift(id);
      localStorage.setItem('nexora_next_recent_' + poolKey, JSON.stringify(recent.slice(0, RECENT_MAX)));
    } catch (e) {}
  }
